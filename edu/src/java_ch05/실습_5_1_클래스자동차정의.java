...
class Car {
    ...
    int speed;           // 속도
	/*
	 * C-세그먼트 (준중형, 소형 패밀리카) > 4,200mm ~ 4,600mm > 약 1,750mm > 현대 아반떼, 기아 K3, 폭스바겐
	 * 골프, 도요타 코롤라
	 * D-세그먼트 (중형차, 패밀리 세단) > 4,600mm ~ 4,900mm > 약 1,800mm ~ 1,850mm >
	 * 현대 쏘나타, 기아 K5, BMW 3시리즈, 벤츠 C클래스 
	 * E-세그먼트 (대형차, 고급 세단) > 4,900mm ~ 5,100mm > 약
	 * 1,850mm ~ 1,900mm > 현대 그랜저, 기아 K8, BMW 5시리즈, 벤츠 E클래스 
	 * F-세그먼트 (초대형차, 플래그십 세단) >
	 * 5,100mm 이상 > 1,900mm 이상 > 제네시스 G90, 벤츠 S클래스, BMW 7시리즈, 아우디 A8
	 */
    ...
    String getSegment() {
    	if(length<4200) {
    		return "B-세그먼트(소형차)";
    	}else if( length<4600){ //'length>=4200'는 위에서 통과되서 왔으니까 필요없음
    		return "C-세그먼트(준중형,소형 패밀리카)";
    	}else if( length<4900) {
    		return "E-세그먼트(중형차, 패밀리 세단)";
    	}else{
    		return "F-세그먼트 (초대형차, 플래그십 세단)";
    	}    	
    }
    
    void checkSpeeding(int speedLimit) {     //checkSpeeding 메소드가 왜 2개인가-->오버로딩이어서
        if (speed > speedLimit) {
...
public class 실습_5_1_클래스자동차정의 {
	public static void main(String[] args) {
        // 자동차 객체 생성
        Car car1 = new Car("Hyundai", "Sonata", 1400,4600,1800, 95); //생성자 작성한 순서로 작성하면 되는건가 oo
        ...
        System.out.println(car1); //1. car1은 string이 아니어서 위에 class method에서 String to String을 통해 String으로 바꿔서 다시 값을 보내줌.
