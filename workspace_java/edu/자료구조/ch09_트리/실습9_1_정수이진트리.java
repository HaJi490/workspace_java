...
//--- 생성자(constructor) ---//
	public ObjectStack5(int capacity) {
		...
		data = new ArrayList<TreeNode5>(capacity);
		top = 0;
	}
...
//--- 스택에 x를 푸시 ---//
	public boolean push(TreeNode5 x) throws OverflowGenericStackException {
		if(isFull()) {
			throw new OverflowGenericStackException("push: overflow");
		}
		data.add(x);
		top++;
		return true;
...
//--- 스택에서 데이터를 피크(peek, 정상에 있는 데이터를 들여다봄) ---//
	public TreeNode5 peek() throws EmptyGenericStackException {
		if(isEmpty()) {
			throw new EmptyGenericStackException("peek: empty");
		}
...
class Tree5 {
	...
	boolean isLeafNode(TreeNode5 current) {// current 가 leaf node(자식노드 x) 인지 조사

	}
	
	boolean isOneChildNode(TreeNode5 current) {//자식 노드가 하나인지 조사
	...
	// non-recursive 코드를 이해하는 학습 필요
	{
		...
		while (true) {
			...
			if (!s.isEmpty()) {
				try {
					CurrentNode = s.pop();
				} catch (ch09_트리.ObjectStack5.EmptyGenericStackException e) {
	...
	boolean insert(int x) {// binary search tree를 만드는 입력 : left subtree < 노드 x < right subtree
		...
		while ( p != null) {
			if(x > p.data) {//x값이 p.data보다 클때
				q =p;
				p = p.RightChild;
			}else {// x값이 p.data보다 작을때
				q =p;
				p = p.LeftChild;
			}
		}
	...
	boolean delete(int x) {// binary search tree에서 임의 값을 갖는 노드를 찾아 삭제한다.
		// 난이도 중상
