...
import ch10_해시.OpenHash2.Bucket;
...
//--- 버킷 ---//
	...
	public OpenHash2(int size) {
		try {
			table = new Bucket[size];// Bucket 객체에 대한 "참조"를 저장할 공간만 만드는 것(그냥 배열)
			/*
			 * 객체 3개를 만들지 않음. Bucket 객체를 담을 수 있는 참조 공간 3개짜리 배열을 만든다
			 * 
			 * // 메모리 상태: 
			 * table[0] = null, 
			 * table[1] = null, 
			 * table[2] = null -> 참조만 만들고 객체x
			 */
			for (int i = 0; i < size; i++)
				table[i] = new Bucket();// 실제로 각 인덱스에 Bucket 객체를 생성해서 넣는 과정(생성자)
			this.size = size;
