 ...
 *  Arrays.sort(numbers); // 정상 작동 > Object타입이지만 실제는 정수여서
 *  
...
public class 실습3_6_1객체배열탐색_Comparable {

	static void showData(String msg, PhyscData2[] dt) {
		System.out.println(msg);
		dt.toString();
	}

	static void sortData(PhyscData2[] dt) {
		for (int i = 1; i < dt.length; i++) {
			int j;
			// 참조변수 tmp에 dt[i]값을 대입
			PhyscData2 tmp = dt[i];
			// j가 점점 앞의 값과 비교함
			// j가 0보다 클때까지, 앞에 수가 비교하는 값(dt[i])보다 크면 종료
			for (j = i; j > 0 && dt[j - 1].name.compareTo(tmp.name) > 0; j--) {
				dt[j] = dt[j - 1];
				dt[j - 1] = tmp;
			}
		}
	...
	static void reverse(PhyscData2[] dt) {
		for (int i = 1; i < dt.length; i++) {
			int j;
			PhyscData2 tmp = dt[i];
			for (j = i; j > 0 && dt[j - 1].name.compareTo(tmp.name) < 0; j--) {
...
//			if(dt[i].name.compareTo(dt[i+1].name) < 0) {
//				swap(dt, i, i+1);
//			}
//		}
	}

//	static void swap(PhyscData2[] dt, int i, int j) {
//		PhyscData2 temp = dt[i];
//		dt[i] = dt[j];
//		dt[j] = temp;
//	}

	static int linearSearch(PhyscData2[] dt, PhyscData2 key) {
		int i = 0;
		while (true) {
			if (i == dt.length)
				return -1;
			if (dt[i].name.compareTo(key.name) == 0)
				return i;
			i++;
		}
	}

	static int binarySearch(PhyscData2[] dt, PhyscData2 key) {
		int pl = 0;
		int pr = dt.length - 1;

		while (pl <= pr) {
			int pc = (pl + pr) / 2;
			if (dt[pc].name.compareTo(key.name) == 0)
				return pc;
			else if (dt[pc].name.compareTo(key.name) > 0)
				pr = pc - 1;
			else if (dt[pc].name.compareTo(key.name) < 0)
				pl = pc + 1;
		}
	...
	public static void main(String[] args) {
		...
		PhyscData2[] data = { new PhyscData2("홍길동", 162, 0.3), new PhyscData2("나동", 164, 1.3),
				new PhyscData2("최길", 152, 0.7), new PhyscData2("홍길동", 162, 0.3), new PhyscData2("박동", 182, 0.6),
				new PhyscData2("박동", 167, 0.2), new PhyscData2("길동", 167, 0.5), };
		showData("정렬전", data);
		sortData(data);// 6장 06-5 단순 삽입 정렬 InsertionSort() 함수로 구현
		showData("정렬후", data);
		reverse(data);
